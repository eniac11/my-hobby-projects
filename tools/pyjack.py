        ...
        # CELLTYPE        
            ...
            for key in ['__code__', '__globals__', '__name__',
                ...
                if orgattr is org_obj:
                    localsmap[key.strip('__')] = new_obj
                else:
                    localsmap[key.strip('__')] = orgattr
...
def _get_self():
    global ___code___map

    frame = _inspect.currentframe()
    code = frame.f_back.f_code
    return ___code___map[code]
...
class _PyjackFuncCode(_PyjackFunc):
    ...
    def __init__(self, fn, proxyfn):
        ...
        ___code___map[proxy.__code__] = self
        self._org___code__ = fn.__code__
        self._proxy___code__ = proxy.__code__
        fn.__code__ = proxy.__code__

        fn.restore = self.restore

    def _process_fn(self, args, kwargs):
        self._fn.__code__ = self._org___code__
        value = self._proxyfn(self._fn, *args, **kwargs)
        self._fn.__code__ = self._proxy___code__
        return value
